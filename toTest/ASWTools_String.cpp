...
//---------------------------------------------------------------------------
bool TStrTool::Contains(std::string const& str, std::string const& substr)
{
    return str.find(substr) != std::string::npos;
}
//---------------------------------------------------------------------------
bool TStrTool::Contains(std::wstring const& str, std::wstring const& substr)
{
    return str.find(substr) != std::wstring::npos;
}
//---------------------------------------------------------------------------
bool TStrTool::ContainsIC(std::string const& str, std::string const& substr)
{
    return ToLower(str).find(ToLower(substr)) != std::string::npos;
}
//---------------------------------------------------------------------------
bool TStrTool::ContainsIC(std::wstring const& str, std::wstring const& substr)
{
...
//---------------------------------------------------------------------------
bool TStrTool::EndsWith(std::string const& str, std::string const& suffix)
{
    return str.size() >= suffix.size() && str.compare(str.size() - suffix.size(), suffix.size(), suffix) == 0;
}
//---------------------------------------------------------------------------
bool TStrTool::EndsWith(std::wstring const& str, std::wstring const& suffix)
{
    return str.size() >= suffix.size() && str.compare(str.size() - suffix.size(), suffix.size(), suffix) == 0;
}
//---------------------------------------------------------------------------
bool TStrTool::EndsWithIC(std::string const& str, std::string const& suffix)
{
    std::string strL = ToLower(str);
    std::string suffixL = ToLower(suffix);
    return strL.size() >= suffixL.size() && strL.compare(strL.size() - suffixL.size(), suffixL.size(), suffixL) == 0;
}
//---------------------------------------------------------------------------
bool TStrTool::EndsWithIC(std::wstring const& str, std::wstring const& suffix)
{
    std::wstring strL = ToLower(str);
    std::wstring suffixL = ToLower(suffix);
    return strL.size() >= suffixL.size() && strL.compare(strL.size() - suffixL.size(), suffixL.size(), suffixL) == 0;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
std::string TStrTool::Join(std::vector<std::string> const& elements, std::string const& separator)
{
    if (elements.empty())
        return "";
...
//---------------------------------------------------------------------------
std::wstring TStrTool::Join(std::vector<std::wstring> const& elements, std::wstring const& separator)
{
    if (elements.empty())
        return L"";

    std::wstring result = elements[0];
    for (size_t i = 1; i < elements.size(); i++)
    {
        result += separator;
        result += elements[i];
...
//---------------------------------------------------------------------------
bool TStrTool::StartsWith(std::string const& str, std::string const& prefix)
{
    return str.size() >= prefix.size() && str.compare(0, prefix.size(), prefix) == 0;
}
//---------------------------------------------------------------------------
bool TStrTool::StartsWith(std::wstring const& str, std::wstring const& prefix)
{
    return str.size() >= prefix.size() && str.compare(0, prefix.size(), prefix) == 0;
}
//---------------------------------------------------------------------------
bool TStrTool::StartsWithIC(std::string const& str, std::string const& prefix)
{
    std::string strL = ToLower(str);
    std::string prefixL = ToLower(prefix);
    return strL.size() >= prefixL.size() && strL.compare(0, prefixL.size(), prefixL) == 0;
}
//---------------------------------------------------------------------------
bool TStrTool::StartsWithIC(std::wstring const& str, std::wstring const& prefix)
{
    std::wstring strL = ToLower(str);
    std::wstring prefixL = ToLower(prefix);
    return strL.size() >= prefixL.size() && strL.compare(0, prefixL.size(), prefixL) == 0;
}
//---------------------------------------------------------------------------
// -Static
