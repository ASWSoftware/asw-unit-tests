...
// Holds every option --help documents, parsed by ParseArguments() and consumed by BuildTestFilter()/RunTests().
struct TCliOptions
{
    bool ListOnly = false;
...
TestFilter BuildTestFilter(TTestHandler& tester, TCliOptions const& options, std::string& filterDescription);
std::optional<int> ParseArguments(int argc, char* argv[], TCliOptions& options);
...
//---------------------------------------------------------------------------
// Returns the exit code the program should terminate with immediately (--version/--help, or a parse/validation
// error), or std::nullopt if 'options' was parsed successfully and the program should proceed to run tests.
std::optional<int> ParseArguments(int argc, char* argv[], TCliOptions& options)
{
...
int main(int argc, char* argv[])
{
    ...
    std::optional<int> const earlyExitCode = ParseArguments(argc, argv, options);
    ...
    try
    {
        ...
        TestFilter const filter = BuildTestFilter(tester, options, filterDescription);
